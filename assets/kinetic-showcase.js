(() => {
  ...
  const cardMarkup=(item,index)=>`<article class="kinetic-carousel-card" data-kinetic-card data-index="${index}" data-slot="3" aria-hidden="true"><button class="kinetic-card-image" type="button" data-kinetic-select="${index}" aria-label="${item[1]}を中央に表示"><img src="${item[0]}" alt="${item[1]} サムネイル" draggable="false"></button><div class="kinetic-card-caption"><small>${item[2]}</small><strong>${item[1]}</strong>${item[3]?`<a href="${item[3]}" tabindex="-1">VIEW <span aria-hidden="true">↗</span></a>`:'<span></span>'}</div></article>`;
  ...
  document.querySelectorAll('[data-kinetic-showcase]').forEach(showcase=>{
    ...
    const render=nextCategory=>{const category=collections[nextCategory]?nextCategory:'lp',items=collections[category];stop();current=0;stage.innerHTML=items.map(cardMarkup).join('');cards=[...stage.querySelectorAll('[data-kinetic-card]')];cards.forEach(card=>card.querySelector('[data-kinetic-select]').addEventListener('click',event=>{if(suppressClick){event.preventDefault();return}goTo(Number(card.dataset.index));start()}));positionCards();requestAnimationFrame(()=>showcase.classList.add('is-ready'));start()};
